...
```txt
GET /products?status=active&branchId=
...
No crear venta en backend por cada click.

Mala idea:

```txt
cada producto → POST /sales/{id}/items
```

Eso mete latencia innecesaria.

Mejor para V1:

```txt
carrito local → POST /sales con items → POST /sales/{id}/complete
```
...
```txt
1. Crear venta draft con carrito.
2. Recibir saleId.
3. Completar venta con payments.
4. Backend descuenta inventario.
5. Backend retorna venta completed.
6. Limpiar carrito.
7. Mostrar éxito.
```
